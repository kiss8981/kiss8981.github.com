...
Expo 개발에서 가장 짜증나는 부분 중 하나인 개발에서는 잘 작동하는 앱이 꼭 프로덕션 빌드를 하면 오류를 뱉는다
아래 사진처럼 앱을 실행 하면 바로 충돌함이 뜨면서 꺼지는 경우는 보통 이부분을 확인해보면 해결된다.
...
React Native에서 Screen 패키지가 없어서 발생하는 오류로 아래와 같이 설치해주면 바로 해결이 가능하다.
...
Expo expo-update를 이용하여 기본적으로 OTA 업데이트를 지원해서 스토어 심사없이 바로 업데이트가 가능한데, 이게 가끔 오류를 만든다.
우선 내가 경험한 것으로는 기존 배포된 앱이 있었는데 새롭게 리펙토링을 진행해서 expo@49 버전으로 업데이트 하고 `eas build`를 사용하여 빌드하고 배포하였을 때 오류가 발생했다.
Expo Update 에서 구버전 코드와 충돌이 일어나서 생기던 현상이였다. 이럴때는 `app.json / app.config.js` 파일에서 아래와 같이 `updates` 기능을 사용하지 않고 배포하면 정상적으로 작동한다.
...
Expo 프로젝트 폴더에서 `npx expo prebuild` 후 Android Studio 에서 `./android` 폴더를 열어 최대한 프로덕션 환경과 비슷하게 테스트 해볼 수 있다.
